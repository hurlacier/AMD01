...
class empruntManager  {
  ...
  // Prépare une requête de type UPDATE.
      $update_emprunt = $this->_db->prepare('UPDATE emprunt SET id_abonne = :id_abonne, id_livre = :id_livre, date_emprunt = :date_emprunt WHERE id_emprunt='.$emprunt->getId_emprunt());
  ...
  public function getListEmprunt() {
    $emprunts = $this->_db->query('SELECT emprunt.id_emprunt AS id_emprunt, abonne.prenom AS prenom, abonne.nom AS nom,
                                  abonne.id_abonne AS id_abonne, livre.titre AS titre, livre.auteur AS auteur, livre.id_livre AS id_livre, emprunt.date_emprunt AS date_emprunt
                      FROM abonne
