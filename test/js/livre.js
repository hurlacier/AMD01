...
// AFFICHAGE DU TABLEAU DE LIVRES ***************************************************************************************************
...
$('#livre').on('submit', function(e){
    ...
    let params='action=ajout_livre&'+$(this).serialize();
...
$("#btnSaveIt").on('click', function (e) {
	...
	let params='action=modification_livre&'+$('#livre_update').serialize()+'&id_livre='+update_id;
    $.post('Traitement.php', // URL du dossier où s'effectue le traitement
            params,  // Valeurs à 'envoyer' contenues dans la variable params
            function (update) {
            	if (update) {
                ...
                $('#livre_update')[0].reset();  // reset du formulaire pour effacer les champs juste pour être propre !!s                
